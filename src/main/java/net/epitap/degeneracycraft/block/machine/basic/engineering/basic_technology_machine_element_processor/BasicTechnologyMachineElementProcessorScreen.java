...
public class BasicTechnologyMachineElementProcessorScreen
        extends AbstractContainerScreen<BasicTechnologyMachineElementProcessorMenu> {

    private static final ResourceLocation TEXTURE =
            new ResourceLocation(Degeneracycraft.MOD_ID,
                    "textures/gui/basic/engineering/basic_technology_machine_element_processor/basic_technology_machine_element_processor_gui.png");
    ...
    public BasicTechnologyMachineElementProcessorScreen(
            BasicTechnologyMachineElementProcessorMenu menu,
            Inventory inv,
            Component title) {
    ...
    protected void renderLabels(GuiGraphics guiGraphics, int pMouseX, int pMouseY) {
        ...
        if (menu.isWorking()) {
            guiGraphics.drawString(this.font, "Work!", 67, 30, 0x00FF00, false);
        } else {
            guiGraphics.drawString(this.font, "Stop!", 67, 30, 0xFF0000, false);
        }
        ...
        if (menu.isForceHalt()) {
            guiGraphics.drawCenteredString(this.font,
                    Component.translatable("screen.degeneracycraft.halt"),
                    133, 66, 0xFFFFFF);
        }

        switch (menu.getHologramLevel()) {
            case 1 -> guiGraphics.drawCenteredString(this.font, "Lv.1", 80, 47, 0xFF0000);
            case 0 -> guiGraphics.drawCenteredString(this.font, "ON", 80, 47, 0x00FF00);
            default -> guiGraphics.drawCenteredString(this.font, "OFF", 80, 47, 0xFF0000);
        }

        if (menu.isInputLocked()) {
            guiGraphics.drawCenteredString(this.font,
                    Component.translatable("screen.degeneracycraft.lock"),
                    43, 66, 0xFFFFFF);
        }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (isMouseOver(mouseX, mouseY, x + HOLOGRAM_X, y + HOLOGRAM_Y)) {
            DCMessages.sendToServer(
                    new DCMachineToggleC2SPacket(menu.getBlockEntity().getBlockPos(), 0)
            );
            return true;
        }
